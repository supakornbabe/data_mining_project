...
let id = 1;
let totalCount = 0;
let tagSet: Map<string, number>;

const ws = fs.createWriteStream(path.resolve(`../data/topic_tags.csv`), {
    encoding: 'utf-8'
});

const stringifier = stringify({
    header: false,
    columns: [ 'id', 'topic_id', 'tag_id' ]
});
stringifier.pipe(ws);
        ...
        try {
            ...
            if (!!!topicIdSet.has(topic_id)) {
                ...
                for (const tag of tags) {
                    stringifier.write([ id, topic_id, tagSet.get(tag) ]);
                    id++;
                }
            }
        } catch (err) {
...
uniqueFileToMap(path.resolve('../data/unique_tag.txt'))
.then(set => {
    tagSet = set;
